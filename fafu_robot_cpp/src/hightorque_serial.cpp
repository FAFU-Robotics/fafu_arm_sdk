...
//  调试板 USB 串口驱动 (1:1 移植自 hightorque_serial.py).
// =============================================================================
...
// saturate_to_i16 定义在 hightorque_protocol.hpp 的 detail 中 (协议层与驱动层
// 共用), 拉进当前命名空间以保持非限定调用.
...
std::optional<MotorState> HightorqueSerial::set_pos_vel_tqe_kp_kd(
    ...
    const int16_t tqe_int = (coeff != 0.0)
        ? saturate_to_i16(static_cast<long long>(tqe_nm / (coeff * 0.01))) : 0;
    // kp/kd 单位 N*m/rad. 换算照抄厂商 SDK motor.cc kp_float2int/kd_float2int
    // (pos_vel_convert_type::radian_2pi 分支): raw = (v / coeff) * 10 * 2pi.
    // 旧实现是 kp 直传 + kd*100, 与厂商公式和 Python 侧 servo_start 都不一致.
    const double  kscale  = (coeff != 0.0) ? (10.0 * 2.0 * kPi / coeff) : 0.0;
    const int16_t kp_int  = saturate_to_i16(std::llround(kp * kscale));
    const int16_t kd_int  = saturate_to_i16(std::llround(kd * kscale));
    auto st = control_call(
        [&](int can_id, double t) {
            return send_can_and_recv_(can_id,
...
double HightorqueSerial::state_age_ms(int motor_id) const {
    std::lock_guard<std::mutex> lk(cache_mtx_);
    auto it = state_cache_.find(motor_id);
    if (it == state_cache_.end() || it->second.rx_time_s <= 0.0) return -1.0;
    return (now_seconds() - it->second.rx_time_s) * 1000.0;
}
